...
export default function AIMechanicScreen() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      text: "Chào Biker! Tôi là Bác sĩ Xe Máy. Xe bạn đang bị xào máy, kêu cò hay dọng dên? Hãy mô tả hoặc bấm nút Camera quay tiếng máy 5 giây cho tôi nghe thử nhé!",
      sender: "bot",
    },
  ]);
  ...
  useEffect(() => {
      ...
      if (user) {
        unsubscribeDoc = onSnapshot(doc(db, "users", user.uid), (docSnap) => {
          if (docSnap.exists() && docSnap.data().bike) {
            ...
            setBikeContext(
              `Khách hàng đang chạy xe: ${bike.brand} ${bike.model}. ODO: ${bike.odo || 0} km. Thay nhớt lần cuối: ${bike.lastOilChangeOdo || 0} km.`,
            );
          } else {
            setBikeContext("");
          }
    ...
    (async () => {
      try {
        ...
        console.log("Lỗi định vị ngầm:", e);
      }
    })();
  ...
  const SYSTEM_PROMPT = `Bạn là Bác sĩ Xe Máy, chuyên gia máy gầm xe máy số 1 Việt Nam.
...
4. Phong cách: Biker bụi bặm.`;
  ...
  const handleSendText = async () => {
    ...
    const aiText = await fetchAITextResponse(
      messages,
      userMsg.text,
      SYSTEM_PROMPT,
    );
  ...
  const openCameraUI = async () => {
    ...
    if (camPermission?.granted && micPermission?.granted) {
      ...
      Alert.alert(
        "Cần quyền",
        "Vui lòng cấp quyền Camera và Micro để A.I nghe tiếng máy!",
      );
    }
  };
      ...
      try {
        ...
        if (video) {
          ...
          setMessages((prev) => [
            ...
            {
              id: Date.now().toString(),
              text: "[Gửi đoạn Video chẩn đoán âm thanh 5 giây]",
              sender: "user",
              isVideo: true,
            },
          ]);
  ...
  const renderTextWithLinks = (text: string, isVideo?: boolean) => {
    ...
    return (
      <Text
        style={[
          styles.messageText,
          isVideo && { fontStyle: "italic", color: "#FFF" },
        ]}
      >
      ...
      >
        ...
        <View
          style={[
            ...
            item.isVideo && styles.videoBubble,
          ]}
        >
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <View style={styles.inputContainer}>
          ...
          <TextInput
            style={styles.input}
            placeholder="Hoặc mô tả bệnh xe tại đây..."
